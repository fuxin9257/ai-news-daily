...
2. **结构化数据**：将收集到的新闻整理为JSON格式，字段包括：id, title, summary, url, source, category(product_release/research_paper/funding/opensource/regulation/community_hot), tags, heat(1-5), date。保存到 E:\AI\news-backup\YYYY-MM-DD\news.json（YYYY-MM-DD为当天日期），同时生成 summary.md 和 report.json。
